"""
Поиск основных драйверов изменения GMV на истории нескольких недель.

Модуль считает вклад каждого среза в недельное изменение GMV, а затем выбирает
компактный набор непересекающихся управленческих сегментов через Set Packing.
"""
...
# 2) отсутствующие недели заменяются на 0, добавляется флаг на пропуск. Это может влиять на оценку Delta GMV для новых и исчезнувших сегментов. Рассмотреть в будущем
            ...
            rows.append(
                {
                    ...
                    "driver_score": float(scores.get(segment_id, 0.0)),
...
def search_drivers(
    ...
    Returns:
        Tuple with final selected drivers, candidate diagnostics and optimization decision log.
...
def build_history_for_selected(panel_df: pd.DataFrame, final_df: pd.DataFrame, total_by_date: pd.Series) -> pd.DataFrame:
    """Подготовить историю недель для выбранных драйверов.
...
def build_driver_analysis_sheet(candidates: pd.DataFrame, final_df: pd.DataFrame, thresholds: DriverThresholds) -> pd.DataFrame:
    """Сформировать лист анализа драйверов, прошедших первичный фильтр.
    ...
    Returns:
        Таблица драйверов, прошедших первичный фильтр, с причиной отсутствия в менеджерском выводе.

    Raises:
        ValueError: Не выбрасывается.

    Examples:
        >>> # analysis_df = build_driver_analysis_sheet(candidates, final_df, DriverThresholds())
    """
    ...
    if candidates.empty or "passes_initial_driver_filter" not in candidates.columns:
        return pd.DataFrame(columns=columns)

    initial_driver_mask = (
        candidates["passes_initial_driver_filter"].eq(True)
        & candidates["abs_delta_gmv"].astype(float).ge(thresholds.min_driver_abs)
    )
    analysis_mask = initial_driver_mask
    ...
    def manager_exclusion_reason(row: pd.Series) -> str:
        ...
        Returns:
            Текст причины для листа анализа драйверов.
        ...
        Examples:
            ...
            'не выбран в итоговый набор драйверов'
        """
        ...
        return classifier_reason if classifier_reason else "не выбран в итоговый набор драйверов"
    ...
    export = pd.DataFrame(
        {
            ...
            "base_driver_score": pd.to_numeric(
                analysis.get("base_driver_score", pd.Series(math.nan, index=analysis.index)),
                errors="coerce",
            ),
            ...
            "driver_score": analysis["driver_score"].astype(float),
            "выбран": analysis.get(
    ...
    return export.sort_values(
        by=["глубина", "driver_score", "Delta GMV", "сегмент"],
        ascending=[False, False, False, True],
    ).reset_index(drop=True)


def highlight_manager_rows_on_driver_analysis(worksheet) -> None:
    """Выделить зелёным строки листа анализа драйверов, попавшие в менеджерский вывод.

    Args:
        worksheet: Лист openpyxl `Анализ драйверов`.
    ...
    Examples:
        >>> # highlight_manager_rows_on_driver_analysis(worksheet)
    """
...
def build_driver_tree_from_excel(
    report_path: str | Path,
    output_path: str | Path | None = None,
    sheet_name: str = "Анализ драйверов",
    dpi: int = 160,
) -> Path:
    """Построить дерево драйверов GMV по листу итогового Excel-файла.
    ...
    Args:
        report_path: Путь к Excel-отчёту с листом анализа драйверов.
    ...
    Examples:
        >>> # build_driver_tree_from_excel('gmv_driver_report_1.xlsx', 'gmv_tree.png')
    """
    ...
    required_columns = {"сегмент", "глубина", "driver_score", "Delta GMV"}
    ...
    nodes["driver_score"] = pd.to_numeric(nodes["driver_score"], errors="coerce")
    ...
    def build_tree_detail_lines(row: pd.Series) -> List[str]:
        ...
        Args:
            row: Строка узла из листа анализа драйверов.
    ...
    for depth in depth_values:
        for node_id in ordered_by_depth[int(depth)]:
            ...
            score_value = _safe_float(node["driver_score"], math.nan)
...
def write_driver_excel(
    ...
    """Записать результат поиска драйверов GMV в Excel.
    ...
    Examples:
        >>> # write_driver_excel('gmv_driver_report.xlsx', thresholds, dims, history_df, panel, candidates, final_df, manager_df, total, dates, dates[-1], coverage, log)
    """
    ...
    params = pd.DataFrame(
        [
            ...
            ("min_driver_abs", thresholds.min_driver_abs),
    ...
    driver_analysis = build_driver_analysis_sheet(candidates, final_df, thresholds)
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        final_export.to_excel(writer, sheet_name="02_Итог_драйверов", index=False)
        driver_analysis.to_excel(writer, sheet_name="Анализ драйверов", index=False)
        ...
        highlight_manager_rows_on_driver_analysis(writer.sheets["Анализ драйверов"])
...
def run_driver_analysis(
    ...
    Examples:
        >>> # result = run_driver_analysis('input.xlsx', 'output.xlsx')
    """
    ...
    final_df, diagnostics, optimization_decision_log = search_drivers(candidates, thresholds, coverage=coverage, dim_cols=dims)
    ...
    if tree_output_path is not None:
        build_driver_tree_from_excel(output_path, tree_output_path)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Поиск основных драйверов изменения GMV на истории недель.")
    ...
    parser.add_argument(
        ...
        help="Путь к PNG, SVG или PDF с деревом листа «Анализ драйверов». По умолчанию создаётся «Граф.png».",
    )
    ...
    parser.add_argument("--min-driver-abs", type=float, default=DriverThresholds.min_driver_abs)
    parser.add_argument("--min-materiality-share", type=float, default=DriverThresholds.min_materiality_share)
    parser.add_argument(
        "--set-packing-gap-tolerance",
        type=float,
        default=DriverThresholds.set_packing_gap_tolerance,
        help="Relative MIP gap tolerance required to mark set-packing optimization as OPTIMAL.",
    )
    parser.add_argument(
        "--max-exact-fallback-size",
        type=int,
        default=DriverThresholds.max_exact_fallback_size,
        help="Maximum conflicting component size allowed for internal exact branch-and-bound fallback.",
    )
    parser.add_argument("--max-manager-facts", type=int, default=DriverThresholds.max_manager_facts)
...
def main() -> None:
    ...
    Examples:
        >>> # python Insight.py --input payoffline_pulse_hier_22_07.xlsx
    """
    ...
    thresholds = DriverThresholds(
        min_driver_abs=args.min_driver_abs,
    ...
    if args.tree_output:
        ...
        if not tree_path.suffix:
            tree_path = tree_path.with_suffix(".png")
        print(f"Дерево драйверов: {tree_path.resolve()}")
    print(f"Кандидатов: {control.get('candidate_count')}")
    print(f"Выбрано драйверов: {control.get('selected_count')}")
