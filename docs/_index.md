...
Актуальность: 2026-08-20. База описывает поиск GMV-аномалий, независимый контур
...
и красные пунктирные линии для всех structural CP выбранной сегментации. В PDF попадают
...
Структурные режимы и окна локального тренда намеренно разделены. Пусть exact DP
восстановил непересекающиеся half-open сегменты:

```text
[0,4), [4,8), [8,13)
```

Они без изменений используются в segment costs, OLS-диагностике, level shift,
slope shift, standard errors, z-score и типизации CP. Для `evaluate_trend()`
строятся отдельные окна уровней GMV:

```text
[0,5), [4,9), [8,13)
```

Внутренняя CP тем самым является последней точкой левого и первой точкой правого
тренда. Для пяти уровней в каждом окне оцениваются четыре недельных изменения;
ни одно изменение не повторяется: первое окно содержит переходы `0→1,...,3→4`,
второе — `4→5,...,7→8`, третье — `8→9,...,11→12`. Тренд по-прежнему оценивается
на уровнях GMV существующей функцией `evaluate_trend()`, а не строится на ряде
дельт.

`trend_segmentation` сохраняет прежние структурные `start/end/start_date/end_date/points`
и отдельно содержит `trend_start/trend_end/trend_start_date/trend_end_date`,
`trend_points`, `trend_changes` и признаки включения левой/правой CP. Это не
позволяет смешать окна классификации тренда с интервалами structural fit.
...
structural type. Для соседних окон уровней GMV с общей CP вызывается существующий
`evaluate_trend()`. Значение True возможно только если structural CP найден, обе
стороны имеют `trend_exists=True`, оба направления принадлежат
`{GROWTH, DECLINE}` и различаются. Поэтому, например,
...
Перекрывающиеся локальные окна при этом не конкатенируются: глобальная проверка
берёт один непрерывный срез исходного GMV-ряда, поэтому общая CP не дублируется.
...
`test_trend_most_recent_cp.py`: **26 тест-методов проходят**. Дополнительно
...
согласованность глобального summary со структурой режимов и CP. Отдельные
регрессии фиксируют окна с общей CP без повторения недельных изменений и
фактический разворот `DECLINE → GROWTH` на 13 точках сегмента «Новая Мода» при
неизменных structural CP/OLS/SE/z-score.
