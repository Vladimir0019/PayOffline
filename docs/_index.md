...
Актуальность: 2026-08-02. База описывает модульную реализацию поиска аномальных
...
выбирает оптимальный в пределах `gap_tolerance` непересекающийся набор
...
9. Excel-контракт состоит из девяти листов; его состав защищён регрессионным тестом.
...
13. Полное перечисление hierarchy-групп ограничено
   `max_hierarchy_descendants = 25`: перебор растёт как `2^n − 1`, превышение
   лимита останавливает расчёт с диагностикой.
